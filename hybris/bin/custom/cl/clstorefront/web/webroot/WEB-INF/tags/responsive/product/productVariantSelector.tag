...
<c:choose>
	...
	<c:when
		test="${product.multidimensional and not empty product.variantMatrix}">
				...
				<c:forEach begin="1" end="${levels}" varStatus="loop">
					...
					<div class=" clearfix">
						...
						<c:choose>
							...
							<c:otherwise>
								<select id="priority${loop.index}"
									class="selectPriority form-control">
									<c:forEach items="${productMatrix}" var="variantCategory">
										<c:url value="${variantCategory.variantOption.url}"
											var="productStyleUrl" />
										<option value="${productStyleUrl}"
											${(variantCategory.variantOption.code eq product.code) ? 'selected="selected"' : ''}>
	...
	<c:otherwise>
					...
					<div class="variant-selector">
						...
						<ul class="variant-list">
							<c:forEach items="${variantStyles}" var="variantStyle">
								...
								<li
									<c:if test="${variantStyle.url eq currentStyleUrl}"> class="active"</c:if>>
									<c:if test="${variantStyle.url eq currentStyleUrl}">
										<div id="currentStyleValue" data-style-value="${styleValue}"></div>
									</c:if> <c:url value="${variantStyle.url}" var="productStyleUrl" /> <a
									href="${productStyleUrl}" class="colorVariant"
									name="${variantStyle.url}"> <c:if
								...
								<c:forEach items="${variantSizes}" var="variantSize">
									...
									<c:forEach items="${variantSize.variantOptionQualifiers}"
										...
										<c:if test="${variantOptionQualifier.qualifier eq 'size'}">
											<c:set var="variantOptionQualifierValue"
												value="${fn:escapeXml(variantOptionQualifier.value)}" />
		...
		<c:if test="${not empty variantOptions}">
			<div class="variant-section">
				<div class="variant-selector">
					<div class="variant-name">
						<label for="Type"><spring:theme
								code="product.variants.type" /><span
